...
(function(){
  ...
  function hideGate(user){
    ...
    if (adminEntry) adminEntry.classList.remove('show');
    if (adminEntry && user && typeof user.jwt === 'function') {
      user.jwt().then(function(token){
        return fetch('/.netlify/functions/admin-status', { headers:{'Authorization':'Bearer '+token} });
      }).then(function(res){ return res.ok ? res.json() : {isAdmin:false}; })
        .then(function(data){ if(adminEntry) adminEntry.classList.toggle('show', !!data.isAdmin); })
        .catch(function(){ if(adminEntry) adminEntry.classList.remove('show'); });
    }

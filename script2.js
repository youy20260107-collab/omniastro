...
(function(){
  ...
  function setSavedCharts(list){
    ...
    (async ()=>{
      ...
      const mergedList = mergeSavedCharts(list, remoteList, mergedDeletedIds);
      if(JSON.stringify(mergedList) !== JSON.stringify(list)){
        // 雲端上有本機還不知道的其他裝置異動：把合併結果寫回本機，畫面呈現最終完整清單
  ...
  async function zmpServerStorageGet(key){
    ...
    try{
      ...
      return data.value;
    }catch(e){ console.error('[命盤雲端同步] 讀取失敗：', e); return null; }
  }
  ...
  async function syncSavedChartsFromServer(){
    ...
    if(cloudNeedsUpdate){
      await zmpServerStorageSet(CHART_SYNC_KEY, { list: mergedList, deletedIds: mergedDeletedIds, updatedAt: ts });
    }
    setChartSyncBadge('synced');
    renderSavedCharts();
  }
  ...
  document.getElementById('importChartsInput')?.addEventListener('change', (e)=>{
    ...
    reader.onload = ()=>{
      try{
        ...
        const toAdd = imported.filter(x=>x && x.id && !existingIds.has(x.id));
        const merged = existing.concat(toAdd);
        setSavedCharts(merged);
        renderSavedCharts();
        showFriendlyError(`匯入完成，新增 ${toAdd.length} 筆資料（重複ID已自動略過）。`, {type:'success'});
      }catch(err){
  ...
  function renderAIQABlock(ctx, panel){
    ...
    async function sendQuestion(question){
      ...
      try{
        ...
        if(currentProvider==='claude') text = await callClaude(apiKey, model);
        else if(currentProvider==='gemini') text = await callGemini(apiKey, model);
        else {
        ...
        else if(status===429) msg = 'API 請求過於頻繁或額度已用完，請稍後再試。';
...
5. 回答請用繁體中文，控制在500字以內，並用「財運面相／手相」「事業面相／手相」「決策風格」「綜合建議」等小標題分段，最後加一句提醒：本分析僅供娛樂與自我覺察參考，重大商業或財務決策仍請以實際規劃與專業意見為準。`;
    ...
    submitBtn.addEventListener('click', async ()=>{
      ...
      try{
        ...
        if(currentProvider==='claude') text = await callClaudeVision(apiKey, model, images, question);
        else if(currentProvider==='gemini') text = await callGeminiVision(apiKey, model, images, question);
        else {
          const baseUrl = providerBaseUrl(cfg, endpoint);
          if(!baseUrl) throw {configError:true, message:'此服務缺少 API 端點設定。'};
        ...
        else if(status===429) msg = 'API 額度已達上限或請求過於頻繁，請稍後再試。';
